...
def load_data(examples, label_list, tokenizer, max_seq_length, batch_size, data_type: DataType, model_type, output_mode="classification"):
    logging.debug("***** Convert Data to Features (Word-Piece Tokenizing) [{}] *****".format(data_type))
    features = convert_examples_to_features(examples,
                                            ...
                                            sep_token_extra=bool(model_type in ['roberta-base']),
                                            # roberta uses an extra separator b/w pairs of sentences, cf. github.com/pytorch/fairseq/commit/1684e166e3da03f5b600dbb7855cb98ddfcd0805
                                            pad_on_left=bool(model_type in ['xlnet-base-cased']),  # pad on the left for xlnet
                                            pad_token=tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0],
                                            pad_token_segment_id=4 if model_type in ['xlnet-base-cased'] else 0,)

    logging.debug("***** Build PyTorch DataLoader with extracted features [{}] *****".format(data_type))
    logging.debug("  Num examples = %d", len(examples))
